...
struct rose_vm {
    ...
    void copy_input_from_other(rose_vm* other);
    void copy_screen_from_other(rose_vm* other);
    ...
    rose_api_error rose_memcpy(uint32_t dest_addr, uint32_t source_addr, uint32_t len);

    rose_api_error rose_memset(uint32_t dest_addr, uint8_t val, uint32_t len);
