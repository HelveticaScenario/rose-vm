...
void rose_rt_base::reset_palette_filter() {
    for (int i = 0; i < ROSE_PALETTE_INDEX_NUM; ++i) {
        palette_filter.begin[i] = (uint8_t) i;
    }
}

void rose_rt_base::reset_palette_transparency() {
...
void rose_rt_base::reset_clipping_region() {
...
void rose_rt_base::reset_pen_color() {
    *pen_color_addr = 6;
}
...
void rose_rt_base::reset_pointer_positions() {
...
void rose_rt_base::reset_btn_states() {
...
void rose_rt_base::reset_mouse_wheel() {
    for (auto it = mouse_wheel.begin; it != mouse_wheel.end; it++) {
        *it = 0;
    }
}

void rose_rt_base::reset_key_states() {
...
void rose_rt_base::copy_screen_from_other(rose_rt* other) {
...
void rose_rt_base::make_mem_ranges() {
    bool hd = meta.hd;
...
rose_rt_base::rose_rt_base(rose_fs* fs) {
    if (fs == NULL) {
...
rose_rt_base::~rose_rt_base() {
    delete mem;
}

bool rose_rt_base::clear() {
    reset_palette_filter();
    reset_palette_transparency();
    reset_clipping_region();
    reset_pen_color();
...
void rose_rt_base::save_input_frame() {
...
void rose_rt_base::update_mouse_pos(int16_t x, int16_t y) {
    int16_t* pointer = (int16_t*) pointer_positions.begin;
    pointer[20] = x;
    pointer[21] = y;
}

void rose_rt_base::update_btn_state(uint8_t btn, bool pressed) {
    rose_set_bit(btn_states.begin, btn, pressed);
}

void rose_rt_base::update_wheel_state(int16_t delta_x, int16_t delta_y, bool inverted) {
...
void rose_rt_base::update_keystate(rose_keycode keycode, bool pressed) {
...
rose_rt::rose_rt(rose_fs* fs): rose_rt_base(fs) {
